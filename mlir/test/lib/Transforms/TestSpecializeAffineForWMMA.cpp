...
void TestSpecializeAffineForWMMA::runOnFunction() {
  ...
  if (getConstantTripCount(computeLoops[ThreadJ]).hasValue()) {
    assert(loopUnrollJamByFactor(
        ...
        getConstantTripCount(computeLoops[ThreadJ]).getValue()).failed() && "Unable to unroll loop, please inspect the IR");
  }

  // Unroll the innermostLoop completely.
  assert(loopUnrollFull(computeLoops[ThreadK]).succeeded() && "Unable to unroll loop, please inspect the IR");
