...
static LogicalResult verify(WMMAMmaF32F32M16N16K16Op op) {
  ...
  for (auto &operand : op->getOpOperands().take_front(numABOperands)) {
    abOpTypes.push_back(operand.get().getType());
  }

  for (auto &operand :
       op->getOpOperands().drop_front(numABOperands).take_front(numCOperands)) {
